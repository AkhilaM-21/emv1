...
/* `to` is the landing page an app's "Explore" button navigates to. Only
   the products that have a page get one — the rest keep an inert link
   rather than routing somewhere that 404s. */
const APPS = [
  { key: 'financials', to: '/products/finance', tab: 'Finance', title: 'Run Your Finances with Greater Control', icon: BadgeDollarSign, color: '#7c3aed', link: 'Explore Finance',
  ...
  { key: 'supply', to: '/products/supply-chain', tab: 'Supply Chain', title: 'Connect Purchasing, Inventory and Warehouses', icon: Truck, color: '#0891b2', link: 'Explore Supply Chain',
  ...
  { key: 'pos', to: '/products/retail-inventory', tab: 'POS', title: 'Connect Every Sale with Your Business Operations', icon: ShoppingCart, color: '#9333ea', link: 'Explore POS',
...
const DashboardERP = ({ inView }) => {
  const { t } = useTranslation();
  return (
  <DashboardWindow title={t('products.dash.erp.title', 'ERP')} inView={inView} sidebarItems={[{icon: <LayoutDashboard size={14}/>, label: t('products.dash.overview', 'Overview')}, {icon: <Briefcase size={14}/>, label: t('products.dash.erp.resources', 'Resources')}, {icon: <Folder size={14}/>, label: t('products.dash.erp.inventory', 'Inventory')}]}>
        ...
        >
          ...
          <div className="emv-featured-apps-container" style={{ '--ac': active.color }}>
            ...
            <div className="emv-agent-studio-card" key={active.key} style={{ '--ac': active.color }}>
              ...
              {active.to ? (
                <Link to={active.to} className="emv-btn-outline-white emv-card-cta">
            ...
            <div className="emv-apps-grid-section">
              <div className="emv-app-detail" key={active.key} style={{ '--ac': active.color }}>
                <div className="emv-app-detail-head">
                  ...
                  {active.to ? (
                    <Link to={active.to} className="emv-app-cta" style={{ '--ac': active.color }}>
                      {active.link} <ArrowRight size={16}/>
                    </Link>
                  ) : (
