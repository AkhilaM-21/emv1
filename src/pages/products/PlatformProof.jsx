...
import { motion, MaskText, Reveal, CountUp, EASE } from './motion';
import {
...
/* ------------------------------------------------------------------
   ...
   ------------------------------------------------------------------ */
const SCATTER = [
  { t: 'Site inspections v7 FINAL.xlsx', m: 'Shared drive · 14 versions', icon: FileSpreadsheet, x: -32, y: -22, r: -7 },
  { t: 'RE: RE: FW: inspection sheet', m: 'Email thread · 41 replies', icon: Mail, x: 26, y: -30, r: 5 },
  { t: 'Site Ops (group)', m: 'WhatsApp · photos, no records', icon: MessageCircle, x: -26, y: 24, r: 4 },
  { t: 'Signed_form_scan_04.pdf', m: 'Paper, scanned, filed nowhere', icon: FileText, x: 30, y: 22, r: -5 },
  { t: '\\\\fileserver\\ops\\2025', m: 'Nobody knows what is current', icon: FolderOpen, x: 0, y: 38, r: 3 },
];
...
const Transformation = () => {
  const { ref, index, pick, bind } = useAutoStep(2, 5200);
  const after = index === 1;
        ...
        <div className="px2-stage">
          ...
          {SCATTER.map((s, i) => (
            <motion.div
              ...
              transition={{ duration: 0.9, delay: after ? i * 0.05 : (SCATTER.length - i) * 0.05, ease: EASE }}
            >
...
const Enterprise = () => {
  ...
  return (
    <section className="pn" id="enterprise">
      <DotField tone="dark" size={30} className="pn-dots fade" />

      <div className="pn-inner">
        <div className="pn-head">
          <Reveal><Kicker tone="dark">14 — Enterprise</Kicker></Reveal>
