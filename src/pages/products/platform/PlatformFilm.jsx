...
/* =====================================================================
   FILM
   ===================================================================== */
const PlatformFilm = ({ playing = true }) => {
  ...
  return (
    <div className="ph-f" ref={ref}>
      {/* ---- application chrome ---- */}
      <div className="ph-f-chrome">
        <span className="ph-f-dots" aria-hidden="true"><i /><i /><i /></span>
        <span className="ph-f-mark">EM</span>
        <span className="ph-f-crumb">
        ...
        <span className="ph-f-omni"><Search size={10} /> Search the ledger <kbd>⌘K</kbd></span>
        <span className={`ph-f-bell ${step >= 6 ? 'on' : ''}`}><Bell size={12} />{step >= 6 && <i />}</span>
        <span className="ph-f-face">RH</span>
      </div>
      ...
      <div className={`ph-f-body ${dim ? 'dim' : ''}`} aria-hidden="true">
        <nav className="ph-f-rail">
          {RAIL.map(([Icon, label, on]) => (
            <span className={`ph-f-rail-i ${on && step >= 2 ? 'on' : ''}`} key={label} title={label}>
              <Icon size={13} strokeWidth={1.8} />
            </span>
          ))}
        </nav>

        <div className="ph-f-main">
          {/* the group position, moved by the posting in scene 4 */}
          <div className="ph-f-kpis">
            <div>
              <span>Revenue</span>
              <Live value={`${CUR} 12.28M`} className="ph-f-num" />
              <em className="up">▲ 4.1%</em>
            </div>
            <div>
              <span>Expenses</span>
              <Live value={step >= 4 ? `${CUR} 7.51M` : `${CUR} 7.42M`} className="ph-f-num" />
              <em className={step >= 4 ? 'move' : ''}>{step >= 4 ? '▲ 1.1%' : '▲ 0.2%'}</em>
            </div>
            <div>
              <span>Cash position</span>
              <Live value={step >= 4 ? `${CUR} 42.7M` : `${CUR} 42.8M`} className="ph-f-num" />
              <em className="up">▲ 2.4%</em>
            </div>
            <div className="ph-f-live"><i />Live</div>
          </div>

          <div className="ph-f-cols">
            {/* --- transaction feed --- */}
            <section className="ph-f-panel">
              <header>
                <b>Recent transactions</b>
                <span className="ph-f-count">{rows.length} today</span>
              </header>
              <div className="ph-f-rows">
                <AnimatePresence initial={false}>
                  {rows.map((r) => {
                    const tag = txnTag(r.id, step);
                    return (
                      <motion.div
                        className={`ph-f-row ${r.id === 'inv' ? 'sel' : ''}`}
                        key={r.id}
                        layout
                      ...
                      >
                        <span className="ph-f-ref">{r.ref}</span>
                        <span className="ph-f-txt"><b>{r.name}</b><i>{r.meta}</i></span>
                        <span className="ph-f-amt">{r.amt}</span>
                        <span className={`ph-f-tag ${tag.tone}`}>{tag.t}</span>
                      </motion.div>
                    );
                  })}
                </AnimatePresence>
            ...
            <section className="ph-f-panel ph-f-insp">
              <AnimatePresence initial={false}>
                {step < 2 ? (
                  <motion.div
                    className="ph-f-queue ph-f-layer"
                    key="queue"
                    initial={{ opacity: 0 }}
                    animate={{ opacity: 1 }}
                    exit={{ opacity: 0 }}
                  ...
                  >
                    <header><b>Approvals</b><span className="ph-f-count">18</span></header>
                    <p>Awaiting your signature</p>
                    <div className="ph-f-queue-rows">
                      {[['Vendor payments', '11'], ['Expense claims', '5'], ['Journals', '2']].map(([l, n]) => (
                        <span key={l}>{l}<i>{n}</i></span>
                      ))}
                    </div>
                    <div className="ph-f-queue-foot"><Clock size={11} /> Oldest 2 days</div>
                  </motion.div>
                ) : (
                  <motion.div
                    className="ph-f-invoice ph-f-layer"
                    key="invoice"
                  ...
                  >
                    <header>
                      <span className="ph-f-inv-k">Invoice</span>
                      <b>INV-2048</b>
                    </header>

                    <div className="ph-f-inv-amt">
                      <span>{CUR}</span> 84,500
                    </div>

                    <dl className="ph-f-inv-fields">
                      ...
                      <div>
                        ...
                        <dd>
                          <AnimatePresence mode="wait" initial={false}>
                            <motion.span
                              key={step >= 3 ? 'ok' : 'wait'}
                              className={`ph-f-tag ${step >= 3 ? 'ok' : 'wait'}`}
                              initial={{ opacity: 0, y: 5 }}
                              animate={{ opacity: 1, y: 0 }}
                              exit={{ opacity: 0, y: -5 }}
                              transition={{ duration: 0.3, ease: EASE }}
                            >
                              {step >= 3 ? <><Check size={9} strokeWidth={4} /> Approved</> : <><i className="ph-f-pulse" /> Pending</>}
                            </motion.span>
                          </AnimatePresence>
                        </dd>
                      </div>
                    ...
                    <div className="ph-f-inv-actions">
                      <motion.span
                        className={`ph-f-btn ${step >= 3 ? 'done' : ''}`}
                        animate={step === 3 ? { scale: [1, 0.94, 1] } : { scale: 1 }}
                        transition={{ duration: 0.4, ease: EASE }}
                      >
                        {step >= 3 ? <><Check size={11} strokeWidth={3.2} /> Approved</> : 'Approve'}
                      </motion.span>
                      <span className="ph-f-btn quiet">Hold</span>
                    </div>
                  </motion.div>
                )}
              </AnimatePresence>
          ...
          <section className="ph-f-panel ph-f-strip">
            <AnimatePresence initial={false}>
              {step < 5 ? (
                <motion.div
                  className="ph-f-ledger ph-f-layer"
                  key="ledger"
                  initial={{ opacity: 0 }}
                  animate={{ opacity: 1 }}
                  exit={{ opacity: 0 }}
                ...
                >
                  <header>
                    <b>General ledger</b>
                    <span className="ph-f-count">Oct 2025 · open</span>
                    {step >= 4 && (
                      <motion.span
                        className="ph-f-tag ok"
                  ...
                  <div className="ph-f-jrow ph-f-jhead">
                    <span>Acct</span>
                  ...
                  <div className="ph-f-jrows">
                    <AnimatePresence initial={false}>
                      {JOURNAL[step >= 4 ? 'posted' : 'prior'].map((j) => (
                        ...
                        >
                          <span className="ph-f-acct">{j.c}</span>
                          <span className="ph-f-jname">{j.n}</span>
                          <span className="ph-f-amt">{j.dr}</span>
                          <span className="ph-f-amt">{j.cr}</span>
                        </motion.div>
                      ))}
                    </AnimatePresence>
                  </div>

                  <div className="ph-f-jfoot">
                    <span><Check size={9} strokeWidth={4} /> Trial balance in balance</span>
                    <b className="ph-f-amt">{step >= 4 ? '84,500' : '1,284,500'}</b>
                    <b className="ph-f-amt">{step >= 4 ? '84,500' : '1,284,500'}</b>
                  </div>
                </motion.div>
              ) : (
                <motion.div
                  className="ph-f-report ph-f-layer"
                  key="report"
                  initial={{ opacity: 0 }}
                  animate={{ opacity: 1 }}
                  exit={{ opacity: 0 }}
                ...
                >
                  <header>
                    <b>Cash flow</b>
                    <span className="ph-f-count">Six months to Oct</span>
                    <span className="ph-f-tag mute">Refreshed just now</span>
                  </header>
                  <div className="ph-f-bars">
                    {bars.map((v, n) => (
                      <motion.i
                        key={n}
                  ...
                  <div className="ph-f-report-side">
                    <span>Net movement</span>
                    <b className="ph-f-num">+{CUR} 3.9M</b>
      ...
      {/* ---- loop position ---- */}
      <div className="ph-f-progress" aria-hidden="true">
        <motion.i
