...
import { motion, useScroll, useTransform, useSpring, useReducedMotion, MaskText, Reveal, EASE } from '../shared/motion';
import {
  Kbd, Chip, DotField, Cursor, Magnet, Cycler,
...
/* =====================================================================
   ...
   The customer wordmarks that used to sit under this section now live
   in the shared trust band immediately below it, so the fold ends on
   the builder rather than on a logo row.
...
/* Five seconds per label, not per beat. Build owns three beats and
   Connect owns one, so a flat interval would leave Build on screen three
   times as long as Connect. Each beat gets 5s divided by however many
   beats its label owns, which makes every label hold for exactly 5s. */
const VERB_BEATS = BEAT_VERB.reduce((m, v) => ({ ...m, [v]: (m[v] || 0) + 1 }), {});
const beatMs = (i) => Math.round(5000 / VERB_BEATS[BEAT_VERB[i]]);
...
const PlatformHero = () => {
  ...
  const { index: beat, pick, ref: seqRef, bind } = useAutoStep(7, beatMs);
